...
// Add comment above fix: Define structural categories for platform capabilities to resolve type errors
const capabilityCategories: Record<string, any> = {
  "core-ide": {
    title: "Core IDE",
    description: "Foundational development tools for biological computing.",
    icon: Code2,
    features: [
      {
        title: "Genome Editor",
        description: "DNA-Lang syntax highlighting and validation.",
        icon: Code2,
        highlights: ["Syntax", "Validation"],
        href: "/ide-platform/editor"
      },
      {
        title: "Quantum Debugger",
        description: "Real-time state tracing and entanglement mapping.",
        icon: Bug,
        highlights: ["Trace", "Breakpoints"],
  ...
  "visual-tools": {
    title: "Visual Tools",
    description: "Intuitive interfaces for complex quantum systems.",
    icon: Workflow,
    features: [
      {
        title: "Circuit Designer",
        description: "Visual builder for genomic quantum circuits.",
        icon: Workflow,
        highlights: ["Visual", "Export"],
      ...
      <div className="max-w-[1200px] mx-auto">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="flex flex-wrap justify-center gap-2 h-auto bg-transparent mb-8">
            {/* Add comment above fix: Cast category to any during iteration to avoid unknown type errors */}
            {Object.entries(capabilityCategories).map(([key, category]: [string, any]) => (
              <TabsTrigger
                key={key}
                value={key}
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap gap-2 mb-4">
                          {feature.highlights.map((highlight: string) => (
                            <Badge key={highlight} variant="secondary" className="text-xs cursor-default">
                              {highlight}
                            </Badge>
                          ))}
                        </div>
